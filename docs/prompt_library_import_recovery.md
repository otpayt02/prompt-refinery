...
`prompt-library/` with the hyphen is treated only as a temporary staging/import source. If useful or unique files appear there, they should be moved into `prompt_library/` before the staging folder is removed.
...
Do not delete potentially useful prompt assets without either:
